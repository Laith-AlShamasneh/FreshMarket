...
public interface IPerson : IRepository<Person>
{
    ...
    Task<Person?> GetByUserIdAsync(long userId, CancellationToken ct = default);
    Task<Person?> GetByPhoneNumberAsync(string phoneNumber, CancellationToken ct = default);
    Task<IReadOnlyList<Person>> SearchByNameAsync(string query, CancellationToken ct = default);
